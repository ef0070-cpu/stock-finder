...
[작성 규칙]
1. 간결함: 모든 내용은 줄글 대신 짧고 명확하게 끊어 쓸 것.
2. 쉬운 용어: 복잡한 지표 수치(예: MACD 시그널선 -291, %K 58.5 등)는 나열하지 말고, 그 지표가
   뜻하는 의미(예: 상승 흐름 시작, 단기 하방 등)만 쉬운 말로 적을 것.
3. 핵심 위주: 부연 설명은 과감히 버리고, 매매에 직접적인 도움이 되는 정보만 남길 것.
4. "📊 차트 흐름"을 쓸 때 이동평균/RSI/MACD/볼린저밴드/스토캐스틱 5개 지표를 모두 살펴서
   종합 판단에 녹여낼 것(숫자 나열 금지, 의미만 쉬운 말로).
5. 손절가·목표가는 "매입가 대비 -15%"처럼 기계적인 고정 비율로 던지지 말 것. 실제 차트상
   지지선·저항선과 기업의 재료·실적(펀더멘털)을 근거로 현실적인 가격대를 제시할 것.
...
2. 💡 AI 분석 포인트
📈 실적 및 호재: [뉴스·실적 기반 호재를 쉬운 말로]
💰 가격 매력도: [애널리스트 목표가 대비 현재가 괴리를 쉬운 말로]
📊 차트 흐름: [이동평균/RSI/MACD/볼린저밴드/스토캐스틱을 종합한 흐름을 쉬운 말로 1~2줄]
